    ...
    public void onBlockBreak(BlockBreakEvent e) {
        ...
        for (GuguSlimefunItem guguSlimefunItem : GuguSlimefunItem.allGuguSlimefunItems) {
            if (guguSlimefunItem.getDropFrom() != null && guguSlimefunItem.getDropFrom().equals(block)) {
                ...
                if (chance >= 100) {
                    guguSlimefunItem.warn("The Block-Breaking drop chance must be between 1-99% on item: " + guguSlimefunItem.getId());
                } else if (chance > ThreadLocalRandom.current().nextInt(100)) {
                    e.getBlock().getWorld().dropItemNaturally(e.getBlock().getLocation(), guguSlimefunItem.getItem().asQuantity(guguSlimefunItem.getDropAmount()));
                    return;
                }
            }
        }
